...
class MetaBoard:
        ...
        for row in range(3):
            for col in range(3):
                if (not self.boards[row][col].is_playable_board()):
                    self._unplayable_board_nums.add(row * 3 + col)
    ...
    def get_unplayable_boards(self):
        return self._unplayable_board_nums

    def update_win_state(self) -> State:
        return self._win_state
